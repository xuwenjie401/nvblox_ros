...
std::filesystem::path addTimestampToFilename(const std::filesystem::path& path) {
  const std::string stem = path.stem().string().empty()
                               ? path.filename().string()
                               : path.stem().string();
  const std::string extension =
      path.extension().empty() ? ".nvblox" : path.extension().string();
  return path.parent_path() / (stem + "_" + timestampMmddHhmm() + extension);
}

std::string resolveMapFilePath(const std::string& path_string,
                               const std::string& default_filename,
                               const bool create_parent_dirs,
                               const bool add_timestamp = false) {
...
class ColorTsdfNode : public rclcpp::Node {
 public:
  ...
  void saveMapIfRequested() {
    ...
    if (config_.map_save_path.empty()) {
      RCLCPP_WARN(get_logger(), "save_map is true but map_save_path is empty");
      return;
    }
    if (mapper_ == nullptr) {
      return;
    ...
    if (mapper_->tsdf_layer().numBlocks() == 0) {
      RCLCPP_INFO(get_logger(),
                  "skipping nvblox map save because TSDF layer is empty");
      map_saved_ = true;
      return;
    }
    ...
    if (mapper_->saveLayerCake(map_save_path)) {
      map_saved_ = true;
      RCLCPP_INFO(get_logger(), "saved nvblox map to %s",
                  map_save_path.c_str());
    } else {
      RCLCPP_ERROR(get_logger(), "failed to save nvblox map to %s",
                   map_save_path.c_str());
    }
  }
 ...
 private:
  ...
  Config loadConfig() {
    ...
    config.load_map = declare_parameter<bool>("load_map", config.load_map);
    config.map_load_path =
        declare_parameter<std::string>("map_load_path", config.map_load_path);
    config.save_map = declare_parameter<bool>("save_map", config.save_map);
    config.map_save_path =
        declare_parameter<std::string>("map_save_path", config.map_save_path);
    return config;
  }
  ...
  std::unique_ptr<nvblox::Mapper> createMapper() {
    ...
    if (config_.map_load_path.empty()) {
      RCLCPP_WARN(get_logger(), "load_map is true but map_load_path is empty");
      return mapper;
    }
    ...
    if (mapper->loadMap(map_load_path,
      ...
      RCLCPP_INFO(get_logger(), "loaded nvblox map from %s",
                  map_load_path.c_str());
    } else {
      RCLCPP_ERROR(get_logger(), "failed to load nvblox map from %s",
                   map_load_path.c_str());
    }
    return mapper;
  }
